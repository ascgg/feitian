...
export function createUser(info, errorHelper, successHelper) {
  const user = db.user;
  const record = db.record;
  const personId = info.personId;
  const account = encryptionHelper.encryptHelper(info.account);
  const password = encryptionHelper.encryptHelper(info.password);
...
export function loginUser(info, errorHelper, successHelper) {
  const user = db.user;
...
export function changePassword(info, errorHelper, successHelper) {
  const user = db.user;
  // If the user is found and the password is right, change to the new one.
  // Else report error.
  const oldPassword = encryptionHelper.encryptHelper(info.oldPassword);
  const newPassword = encryptionHelper.encryptHelper(info.newPassword);
  const account = encryptionHelper.encryptHelper(info.account);
  user.findAndCountAll({ where: { account: account } }).then(function (result) {
  	if (result.count === 1) {
